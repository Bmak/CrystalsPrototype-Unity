...
public abstract class UITweener : MonoBehaviour
{
	...
	[HideInInspector] public float stopPoint = -1f;
	...
	protected void DoUpdate ()
	{
		...
		// OCTOBOX MODIFICATION
		if ((style == Style.Once) && (duration == 0f ||  mFactor > 1f || mFactor < 0f || (stopPoint > 0f && (mAmountPerDelta > 0f && mFactor > stopPoint || mAmountPerDelta < 0f && mFactor < stopPoint))))
		{
			mFactor = Mathf.Clamp01(mFactor);
			if (stopPoint > 0f)
			{
				mFactor = Mathf.Clamp(mFactor, mAmountPerDelta > 0f ? 0f : stopPoint, mAmountPerDelta > 0f ? stopPoint : 1f);
			}
			mFactor = Mathf.Clamp01(mFactor);
			Sample(mFactor, true);
			enabled = false;
			...
			if (duration == 0f || stopPoint > 0f || (mFactor == 1f && mAmountPerDelta > 0f || mFactor == 0f && mAmountPerDelta < 0f))
				enabled = false;			
		}
