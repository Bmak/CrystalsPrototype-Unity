...
public class UIDragDropItem : MonoBehaviour
{
	...
	private const float MAX_HORIZONTAL_DRAG_DEG = 90f;
	private const float MIN_HORIZONTAL_DRAG_DEG = 82.5f;
	private const float MIN_VERTICAL_DRAG_DEG = 0f;
	private const float MAX_VERTICAL_DRAG_DEG = 7.5f;
...
#region Common functionality
	...
	protected virtual void OnDragStart ()
	{
		...
		if (restriction != Restriction.None)
		{
			if (restriction == Restriction.Horizontal)
			{
				...
				// OCTOBOX MODIFICATION: Used to use 45 degree check
				float rad = Mathf.Atan2(Mathf.Abs (delta.y),Mathf.Abs(delta.x));

				float deg = rad * (180 / Mathf.PI);

				if (deg >= MIN_HORIZONTAL_DRAG_DEG && deg <= MAX_HORIZONTAL_DRAG_DEG) return;
			}
			else if (restriction == Restriction.Vertical)
			{
				...
				if (deg >= MIN_VERTICAL_DRAG_DEG && deg <= MAX_VERTICAL_DRAG_DEG) return;
			}
	...
	// OCTOBOX MODIFICATION: pass custom data to a cloned object
	protected virtual void OnClonedFrom(UIDragDropItem other) { }
