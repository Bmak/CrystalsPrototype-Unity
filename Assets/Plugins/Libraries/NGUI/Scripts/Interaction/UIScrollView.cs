...
public class UIScrollView : MonoBehaviour
{
	...
	// for float point comparison operations
	private const float SCROLL_PRECISION = 0.01f;
	...
	protected virtual bool shouldMove
	{
		get
		{
			...
			{
				// OctoBox Modification
				// check whether content fits with precision
				if (b.min.x - clip.x + hx < SCROLL_PRECISION) return true;
				if (b.max.x - clip.x - hx > SCROLL_PRECISION) return true;
				// end of OctoBox Modification
			}
			...
			if (canMoveVertically)
			{
				// OctoBox Modification
				// check whether content fits with precision
				if (b.min.y - clip.y + hy < SCROLL_PRECISION) return true;
				if (b.max.y - clip.y - hy > SCROLL_PRECISION) return true;
				// end of OctoBox Modification
			}
			return false;
		}
