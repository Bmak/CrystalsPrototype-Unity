		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
				...
				else if (mOverflow == Overflow.ResizeFreely)
				{
					...
					// OCTOBOX Modification 
					/* NGUI label mode ResizeFreely rounds down when it calculates the required text field 
                     * width this can cause the text to wrap unexpectedly, especially with certain alignments/pivot 
                     * points such as center. Adding 1 stops the character wrapping and keeps the proper character spacing.
                     * */

					mWidth = Mathf.Max(minWidth, Mathf.RoundToInt(mCalculatedSize.x + 1));

					// end of OCTOBOX Modification 
				...
				// OCTOBOX MODIFICATION
				else if (overflowMethod == Overflow.TruncateWithEllipsis)
				{
					string originalString = mText;
					string truncatedString = "";
					// If name is too long, remove letters and substitute ellipsis
					// First remove last 3 letters, add ellipsis, try again
					int lettersToTrim = Ellipsis.Length;

					while (!fits)
					{
						if (originalString.Length <= lettersToTrim) {
							mText = Ellipsis;    // Apparently couldn't fit in any letter at all, so just substitute ellipsis for name.  This situation should never happen.
							break;
						} else {
							truncatedString = originalString.Substring(0, originalString.Length - lettersToTrim) + Ellipsis;
							lettersToTrim++;
						}

						fits = NGUIText.WrapText(truncatedString, out mProcessedText, true);
					}
				}
				// END OCTOBOX MODIFICATION
				else
